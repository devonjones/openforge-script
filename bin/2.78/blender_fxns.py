...
def union(aobject, bobject, operation='UNION', solver='CARVE'):
...
def unionize(base, objs, operation='UNION', solver='CARVE'):
	for i in objs:
		if i != base:
				bpy.ops.ed.undo_push()
				union(base, i, operation=operation, solver=solver)

def dist_unionize(base, objs, operation='UNION', solver='CARVE'):
	for i in objs:
		if i != base:
				bpy.ops.ed.undo_push()
				union(i, base, operation=operation, solver=solver)

def matrix_unionize(bases, objs, operation='UNION', solver='CARVE'):
	...
	for b, o in zip(bases, objs):
		bpy.ops.ed.undo_push()
		union(b, o, operation=operation, solver=solver)
...
class CurvedWall(object):
	def __init__(self, radius, width, height,
		...
		self.wall = Wall(self.length, width, height,
			...
			testmeshes=testmeshes, solver=solver)
	...
	def unionize(self,retries=0):
		while len(self.active_blocks) > 0:
			...
			try:
				...
				union(self.mortar(), block.get_block(), solver=self.solver)
...
class OpenLockPort(object):
	def __init__(self, obj, layout=None, rot=None, x=None, y=None, negative_name='Negative', neg=True, solver='CARVE'):
	...
	def apply_layout(self, layout):
		...
		if layout['rot'] == 0:
			...
			if layout.get('neg', True):
				...
				move(self.negative(), layout['x'] * 25.4, layout['y'] * 25.4)
		bpy.ops.ed.undo_push()
		union(self.get_object(), self.port_block(), operation='UNION', solver=self.solver)
		bpy.ops.ed.undo_push()
		union(self.get_object(), self.negative(), operation='DIFFERENCE', solver=self.solver)
...
def create_base(base, flourish, template, subname, solver='CARVE'):
	xcopies = 3
	xtrans = [25,0,-25]
	...
	for base in bases:
		base.hide = False
		unionize(base, [template], operation='INTERSECT', solver=solver)
		unionize(base, [flourish], operation='UNION', solver=solver)

def deselect_all():
	for o in bpy.data.objects:
		o.select = False

def upright_bases(bases, negative,xan=0,yan=90,xdist=2,ydist=0, solver='CARVE'):
	for base in bases:
		base.hide = False
		new = copy_object(base)
		...
		unionize(base,[negative], operation='DIFFERENCE', solver=solver)
		unionize(new,[negative], operation='DIFFERENCE', solver=solver)
		deselect_all()
		new.select = True
		base.select = True
