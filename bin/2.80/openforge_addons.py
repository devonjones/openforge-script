...
class BaseObject(object):
	...
	def select(self):
		self.context.view_layer.objects.active = self.get_object()
...
def union(op, context, aobject, bobject, operation='UNION'):
...
def dist_unionize(op, context, objs, operation='UNION'):
	base = context.object
	for obj in objs:
		if obj != base:
			bpy.ops.ed.undo_push()
			union(op, context, obj, base, operation=operation)
...
class BaseBoolean(object):
	...
	def unionize(self, context, objs):
		base = BaseObject(context.view_layer.objects.active, context)
